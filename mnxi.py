...
methods = {
#	"c" : [ "rel", "%SDCC -c -o %DSTFILE %SRCFILE -mz80 --no-std-crt0 --opt-code-speed --Werror --peep-asm --peep-return  --max-allocs-per-node200000" ],
	"c" : [ "rel", "%SDCC -c -o %DSTFILE %SRCFILE -mz80 --no-std-crt0 --opt-code-speed --Werror --peep-asm --peep-return" ],
	"s" : [ "rel", "%SDASZ80 -xlos -g %DSTFILE %SRCFILE" ],
	"ihx" : [ "ihx", "%SDCC -mz80 --no-std-crt0 --opt-code-speed --nostdlib --code-loc 0x2100 -Wl -b_HEADER=0x2000 %REL"],
	"tap" : [ "tap", "%MACKAREL crt0.ihx %DSTFILE Engarde loader.scr -nosprestore -noei"],
	"dot" : [ "dot", "%IHX2BIN crt0.ihx %DSTFILE"]
	}

# If you want to add tags, just add keys here,
# like "SDCC": "../sdcc350/bin/sdcc":
objectlists = {
	"SDCC": "Y:\\Documents\\Software\\vscode_zx\\SDCC\\bin\\sdcc.exe",
	"SDASZ80": "Y:\\Documents\\Software\\vscode_zx\\SDCC\\bin\\sdasz80.exe",
	"MACKAREL": "Y:\\Documents\\Software\\vscode_zx\\Tools\\mackarel\\mackarel.exe",
	"IHX2BIN": "Y:\\Documents\\Software\\vscode_zx\\repos\\specnext\\tools\\ihx2bin.exe"
}
